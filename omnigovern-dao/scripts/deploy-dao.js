...
async function main() {
  ...
  const token = await OmniGovernDAOToken.deploy(hundredTokens);
  console.log("OmniGovernDAOToken deployed to: " + token.address);
  ...
  // Now deploy the DAO
  const OmniGovernDAO = await hre.ethers.getContractFactory("OmniGovernDAO");
  const dao = await OmniGovernDAO.deploy(token.address, lzAddress, [10112, 10106]); // fantom + avalanche
  console.log("OmniGovernDAO deployed to: " + dao.address);
}
